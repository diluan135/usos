export default {
    ...
    async getPagination(dataStart, dataEnd) {
        ...
        try {
            const response = await axios.get(`/api/uso_cartao_2020/pagination`, {
                params: {
                    dataInicio: this.dataStart || '2020-01-01',
                    dataFim: this.dataEnd || '2020-12-31',
                },
            });
    ...
    async getUso(pageNumber, dataStart, dataEnd, qttPerPage, order) {
        try {
            ...
            const apiResponse = await axios.get(`/api/uso_cartao_2020`, {
                params: {
                    page: pageNumber,
                    dataInicio: dataStart || '2020-01-01',
                    dataFim: dataEnd || '2020-12-31',
            ...
            if (!(this.pagination > 0)) {
                try {
                    const paginationResponse = await axios.get(`/api/uso_cartao_2020/pagination`, {
                        params: {
                            page: pageNumber,
                            dataInicio: this.dataStart || '2020-01-01',
                            dataFim: this.dataEnd || '2020-12-31',
    ...
    async encontrarUsuarioPorCartaoId(id){
        try {
            const response = await axios.get(`/api/uso_cartao_2020/${id}`);
            return response;
        } catch (error) {
    ...
    async getUsoUsuario(id, pageNumber){
        try {
            const response = await axios.get(`/api/uso_usuario_2020/${id}?page=${pageNumber}`);
            return response;
        } catch (error) {
    ...
    async getUsuarioLinha(id, pageNumber, dataStart, dataEnd, qttPerPage) {
        try {   
            ...
            const response = await axios.get(`/api/uso_cartao_linha_2020/${id}?page=${pageNumber}`, {
                params: {
                    page: pageNumber,
                    dataInicio: this.dataStart || '2020-01-01',
                    dataFim: this.dataEnd || '2020-12-31',
