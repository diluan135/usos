export default {
    ...
    async getPagination(dataStart, dataEnd) {
        ...
        try {
            const response = await axios.get(`/api/uso_cartao_2019/pagination`, {
                params: {
                    dataInicio: this.dataStart || '2019-01-01',
                    dataFim: this.dataEnd || '2019-12-31',
                },
            });
    ...
    async getUso(pageNumber, dataStart, dataEnd, qttPerPage, order) {
        try {
            console.log(pageNumber, dataStart, dataEnd, qttPerPage, order);
            if (this.dataStart != dataStart || this.dataEnd != dataEnd) {
                this.dataStart = dataStart;
                this.dataEnd = dataEnd;
            }

            const apiResponse = await axios.get(`/api/uso_cartao_2019`, {
                params: {
                    page: pageNumber,
                    dataInicio: dataStart || '2019-01-01',
                    dataFim: dataEnd || '2019-12-31',
            ...
            if (!(this.pagination > 0)) {
                try {
                    const paginationResponse = await axios.get(`/api/uso_cartao_2019/pagination`, {
                        params: {
                            page: pageNumber,
                            dataInicio: this.dataStart || '2019-01-01',
                            dataFim: this.dataEnd || '2019-12-31',
    ...
    async encontrarUsuarioPorCartaoId(id) {
        try {
            const response = await axios.get(`/api/uso_cartao_2019/${id}`);
            return response;
        } catch (error) {
    ...
    async getUsoUsuario(id, pageNumber) {
        try {
            const response = await axios.get(`/api/uso_usuario_2019/${id}?page=${pageNumber}`);
            return response;
        } catch (error) {
    ...
    async getUsuarioLinha(id, pageNumber, dataStart, dataEnd, qttPerPage) {
        try {   
            ...
            const response = await axios.get(`/api/uso_cartao_linha_2019/${id}?page=${pageNumber}`, {
                params: {
                    page: pageNumber,
                    dataInicio: this.dataStart || '2019-01-01',
                    dataFim: this.dataEnd || '2019-12-31',
